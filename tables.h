...
// This struct is a value on its own, a singly-linked list, it holds the
// allocation table
...
// This function is used to dump the static allocation table (the allocation
// table is static, not the allocated values!)
void
dump_memory (void)
{
  for (struct Memory *mem = MEMORY; mem != NULL; mem = mem->next)
    {
      if (!mem->free)
        {
          free (mem->memv);
...
// This is another list structure, it holds the symbols and their values. All
// symbols are strings
...
{
  for (struct Symtab *stab = SYMTAB; stab != NULL; stab = stab->next)
    {
